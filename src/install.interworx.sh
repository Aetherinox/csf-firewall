...
ARG_SCRIPT="${1:-install.interworx.sh}"
ARG_PANEL="${2:-Interworx}"
...
if [ -f "./${CSF_AUTO_INTERWORX}" ]; then
    info "    Found ${bluel}${CSF_AUTO_INTERWORX}${greym}; applying chmod 0700"
    run chmod -v 700 "./${CSF_AUTO_INTERWORX}"

    if [ -x "./${CSF_AUTO_INTERWORX}" ]; then
        info "    Running ${bluel}${CSF_AUTO_INTERWORX}${greym} with version ${bluel}${OLDVERSION}${greym}"
        run "./${CSF_AUTO_INTERWORX}" "${OLDVERSION}"
    else
        error "    File exists but is not executable: ${redl}${CSF_AUTO_INTERWORX}${greym}"
    fi
else
    error "    File not found: ${redl}${CSF_AUTO_INTERWORX}${greym}"
fi
...
# #
#	@app			Interworx
#	@desc			Set specific Interworx integration
# #

run mkdir -v -m 0600 /usr/local/interworx/plugins/configservercsf /usr/local/interworx/html/configserver
run chmod -v 0711 /usr/local/interworx/html/configserver
...
if [ -e "/usr/local/interworx/include/Ctrl/Nodeworx/Firewall.php.orig" ]; then
    run mv /usr/local/interworx/include/Ctrl/Nodeworx/Firewall.php.orig /usr/local/interworx/include/Ctrl/Nodeworx/Firewall.php 
    run chown iworx:iworx /usr/local/interworx/include/Ctrl/Nodeworx/Firewall.php
    run chmod 600 /usr/local/interworx/include/Ctrl/Nodeworx/Firewall.php
fi

run chattr -ia /etc/apf/apf
if [ -e "/etc/apf/apf.old" ]; then
    run cp -avf apf_stub.pl /etc/apf/apf
else
    run mv /etc/apf/apf /etc/apf/apf.old
    run cp -avf apf_stub.pl /etc/apf/apf
fi
run chmod 750 /etc/apf/apf
