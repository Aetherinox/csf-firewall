...
ARG_SCRIPT="${1:-install.cyberpanel.sh}"
ARG_PANEL="${2:-CyberPanel}"
...
if [ -f "./${CSF_AUTO_CYBERPANEL}" ]; then
    info "    Found ${bluel}${CSF_AUTO_CYBERPANEL}${greym}; applying chmod 0700"
    run chmod -v 700 "./${CSF_AUTO_CYBERPANEL}"

    if [ -x "./${CSF_AUTO_CYBERPANEL}" ]; then
        info "    Running ${bluel}${CSF_AUTO_CYBERPANEL}${greym} with version ${bluel}${OLDVERSION}${greym}"
        run "./${CSF_AUTO_CYBERPANEL}" "${OLDVERSION}"
    else
        error "    File exists but is not executable: ${redl}${CSF_AUTO_CYBERPANEL}${greym}"
    fi
else
    error "    File not found: ${redl}${CSF_AUTO_CYBERPANEL}${greym}"
fi
...
run mkdir -vp /usr/local/CyberCP/public/static/configservercsf/
run cp -avf csf/* /usr/local/CyberCP/public/static/configservercsf/
run cp -avf csf/* cyberpanel/configservercsf/static/configservercsf/
run chmod 755 /usr/local/CyberCP/public/static/configservercsf/

run cp cyberpanel/cyberpanel.pl /usr/local/csf/bin/
run chmod 700 /usr/local/csf/bin/cyberpanel.pl
run cp -avf cyberpanel/configservercsf /usr/local/CyberCP/

run mkdir /home/cyberpanel/plugins
run touch /home/cyberpanel/plugins/configservercsf
...
# #
#	@app			CyberPanel
#	@desc			This is for older versions of Cyberpanel, not needed in newer releases.
#   
#                   Open
...
# #
#	@app			CyberPanel
#	@desc			For newer versions of CyberPanel.
#   
#                   Open
#		                /usr/local/CyberCP/baseTemplate/templates/baseTemplate/index.html
#	                Find
...
# #
#	@app			CyberPanel
#	@desc			In Django 2.x+, URL routing switched from the old url() function to the newer path().
...
# #
#	@app			CyberPanel
#	@desc		    Saving form on "Firewall Configuration" page will toss the following error:
