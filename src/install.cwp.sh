...
if [ -e "/usr/local/cwpsrv/" ]; then
	if [ -e "/usr/local/cwpsrv/htdocs/resources/admin/modules/csf.orig.php" ]; then
		run mv -fv /usr/local/cwpsrv/htdocs/resources/admin/modules/csf.orig.php /usr/local/cwpsrv/htdocs/resources/admin/modules/csf.php
	fi
fi
...
run copi "cwp/csfofficial.php" "/usr/local/cwpsrv/htdocs/resources/admin/modules/"
run copi "cwp/csf.pl" "/usr/local/cwpsrv/htdocs/resources/admin/modules/"
run copi "cwp/ajax_csfframe.php" "/usr/local/cwpsrv/htdocs/resources/admin/addons/ajax/"
run copi "cwp/configserver.php" "/usr/local/cwpsrv/htdocs/resources/admin/include/"
...
if [ ! -d "$CSF_CWP_PATH_DESIGN" ]; then
    run mkdir -v -m 0600 "$CSF_CWP_PATH_DESIGN"
...
if lsattr "$CSF_CWP_PATH_DESIGN" 2>/dev/null | grep -q 'i'; then
	...
	run chattr -R -i "$CSF_CWP_PATH_DESIGN"
...
run copi "$CSF_CWP_FOLD_SRC" "$CSF_CWP_PATH_DESIGN"
cwp_copy_status=$?
