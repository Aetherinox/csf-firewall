...
ARG_SCRIPT="${1:-install.cpanel.sh}"
ARG_PANEL="${2:-cPanel}"
...
# #
#	@app			cPanel / WHM
#	@desc			Update shebang for several cpanel scripts
#						Search for
#							#!/usr/bin/perl
#						Replace with
#							#!/usr/local/cpanel/3rdparty/bin/perl
#	
#					1st of 2 replacements
# #

if [ -e "/usr/local/cpanel/3rdparty/bin/perl" ]; then
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' auto.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' cpanel/csf.cgi
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' csf.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' csftest.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' lfd.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' os.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' pt_deleted_action.pl
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' regex.custom.pm
    run sed -i 's%^#\!/usr/bin/perl%#\!/usr/local/cpanel/3rdparty/bin/perl%' webmin/csf/index.cgi
fi
...
if [ "$dr" = "false" ]; then
	...
	cp -avf defaults.txt /usr/local/csf/lib/
...
if [ -f "./${CSF_AUTO_CPANEL}" ]; then
    info "    Found ${bluel}${CSF_AUTO_CPANEL}${greym}; applying chmod 0700"
    run chmod -v 700 "./${CSF_AUTO_CPANEL}"

    if [ -x "./${CSF_AUTO_CPANEL}" ]; then
        info "    Running ${bluel}${CSF_AUTO_CPANEL}${greym} with version ${bluel}${OLDVERSION}${greym}"
        run "./${CSF_AUTO_CPANEL}" "${OLDVERSION}"
    else
        error "    File exists but is not executable: ${redl}${CSF_AUTO_CPANEL}${greym}"
    fi
else
    error "    File not found: ${redl}${CSF_AUTO_CPANEL}${greym}"
fi

# #
#	@app			cPanel / WHM
#	@desc			Set various cpanel file permissions, create directories, copy files.
# #

run mkdir /usr/local/cpanel/whostmgr/docroot/cgi/configserver
run chmod 700 /usr/local/cpanel/whostmgr/docroot/cgi/configserver
run mkdir /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf
run chmod 700 /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf

run cp -avf cpanel/csf.cgi /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf.cgi
run chmod -v 700 /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf.cgi

run cp -avf csf/ /usr/local/cpanel/whostmgr/docroot/cgi/configserver/
run cp -avf cpanel/Driver /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/
run cp -avf ui/images/icon.gif /usr/local/cpanel/whostmgr/docroot/themes/x/icons/csf.gif
run cp -avf cpanel/csf.tmpl /usr/local/cpanel/whostmgr/docroot/templates/

# #
#	@app			cPanel / WHM
#	@desc			Reads the cPanel version from /usr/local/cpanel/version and extracts the second number (the build version).
#						› If cpanel version is < 65, update file cpanel/csf.conf to use target=mainFrame
#						› If cpanel version is 65+, update file cpanel/csf.conf to use target=_self
# #

VERSION=`cat /usr/local/cpanel/version | cut -d '.' -f2`
if [ "$VERSION" -lt "65" ]; then
    run sed -i "s/^target=.*$/target=mainFrame/" cpanel/csf.conf
    echo "cPanel v$VERSION, target set to mainFrame"
else
    run sed -i "s/^target=.*$/target=_self/" cpanel/csf.conf
    echo "cPanel v$VERSION, target set to _self"
fi

# #
#	@app			cPanel / WHM
#	@desc			Copy local cpanel/csf.conf to cpanel files
#					Copy local cpanel/upgrade.sh to cpanel files
#					Chmod copied upgrade.sh
# #

run cp -avf cpanel/csf.conf /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/csf.conf
run cp -avf cpanel/upgrade.sh /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/upgrade.sh
run chmod 700 /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/upgrade.sh

# #
#	@app			cPanel / WHM
#	@desc			AppConfig support
#						If /usr/local/cpanel/bin/register_appconfig exists (newer cPanel):
#							› Copy CSF AppConfig driver to cpanel's driver dir
#							› Touch driver folder, update timestamp
#							› Register CSF AppConfig driver with cpanel
#							› Remove legacy CSF CGI installer from WHM
#						If cPanel using older version without AppConfig Support
#							› Install legacy CSF CGI addon (addon_csf.cgi) to WHM
#							› Copy CSF UI files into WHM CGI directory
#							› Fix permissions on /var/cpanel/apps
#							› Copy old format CSF config to /var/cpanel/apps/csf.conf
#							› Fix perms on /var/cpanel/apps/csf.conf
# #

if [ -e "/usr/local/cpanel/bin/register_appconfig" ]; then
    run /bin/cp -af /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/Driver/* /usr/local/cpanel/Cpanel/Config/ConfigObj/Driver/
    run /bin/touch /usr/local/cpanel/Cpanel/Config/ConfigObj/Driver

    run /usr/local/cpanel/bin/register_appconfig /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf/csf.conf
    run /bin/rm -f /usr/local/cpanel/whostmgr/docroot/cgi/addon_csf.cgi
    run /bin/rm -Rf /usr/local/cpanel/whostmgr/docroot/cgi/csf
else
    run cp -avf cpanel/csf.cgi /usr/local/cpanel/whostmgr/docroot/cgi/addon_csf.cgi
    run chmod -v 700 /usr/local/cpanel/whostmgr/docroot/cgi/addon_csf.cgi
    run cp -avf csf/ /usr/local/cpanel/whostmgr/docroot/cgi/
    if [ ! -d "/var/cpanel/apps" ]; then
        run mkdir /var/cpanel/apps
        run chmod 755 /var/cpanel/apps
    fi
    run /bin/cp -avf cpanel/csf.conf.old /var/cpanel/apps/csf.conf
    run chmod 600 /var/cpanel/apps/csf.conf
fi
...
#							#!/usr/bin/perl
#	
#					2nd of 2 replacements
# #

if [ -e "/usr/local/cpanel/3rdparty/bin/perl" ]; then
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' auto.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' cpanel/csf.cgi
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' csf.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' csftest.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' lfd.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' os.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' pt_deleted_action.pl
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' regex.custom.pm
    run sed -i 's%^#\!/usr/local/cpanel/3rdparty/bin/perl%#\!/usr/bin/perl%' webmin/csf/index.cgi
fi
